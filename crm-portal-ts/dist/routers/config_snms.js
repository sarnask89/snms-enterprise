...
function parseNumberPlanDocType(value, fallback = NumberPlanDocType.invoice) {
    const candidate = String(value ?? "").trim();
    return Object.values(NumberPlanDocType).includes(candidate) ? candidate : fallback;
}
function serializeDivision(row) {
    return {
        id: row.id,
        name: row.name,
...
function serializeNumberPlan(row) {
    return {
        id: row.id,
        name: row.name,
        docType: row.docType,
...
//# sourceMappingURL=config_snms.js.map