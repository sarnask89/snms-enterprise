...
async function mergeMessageContent(templateId, subject, body) {
...
router.put("/messages/:id", async (req, res) => {
    try {
        ...
        const templateId = parseOptionalInteger(req.body?.templateId);
        const merged = await mergeMessageContent(templateId ?? row.templateId, parseOptionalString(req.body?.subject) ?? row.subject, parseOptionalString(req.body?.body) ?? row.body);
        if ("error" in merged) {
            return res.status(400).json({ message: merged.error });
        }
        const status = req.body?.status === MessageStatus.sent || req.body?.sent === true
...
//# sourceMappingURL=snms_entities.js.map