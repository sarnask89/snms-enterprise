...
import { CustomerDevice, IpNetwork, NetDevice, NetworkDeviceAccessProfile, NetworkDiscoveryRecord, NetworkDiscoverySession, } from "../models/network.js";
import { DASAN_DEFAULT_SSH_PORT, DISCOVERY_DRIVER_DASAN, DISCOVERY_DRIVER_MIKROTIK, runDiscoveryScanForDevice, serializeDiscoveryRecord, testDiscoveryAccessProfile, } from "../services/network_discovery_live.js";
...
function serializeCustomerDevice(device) {
    return {
        id: device.id,
...
function serializeIpNetwork(network, sourceDeviceId) {
    return {
        id: network.id,
        name: network.name,
        cidr: network.cidr,
...
function serializeAccessProfile(profile) {
    let metadata = null;
    if (profile.metadataJson) {
        try {
            metadata = JSON.parse(profile.metadataJson);
        }
        catch {
            metadata = null;
        }
...
function serializeDiscoverySession(session) {
    let summary = null;
    if (session.summary) {
        try {
...
function getDefaultPort(driver) {
...
router.put("/access-profiles/:id", async (req, res) => {
    try {
        ...
        const metadata = profile.metadataJson ? JSON.parse(profile.metadataJson) : {};
        const useTls = parseOptionalBoolean(req.body?.useTls);
        if (useTls !== undefined) {
            metadata.useTls = useTls;
        }
...
router.post("/sessions/:id/auto-import", async (req, res) => {
    try {
        ...
        const result = await autoImportDiscoverySession(records, {
...
router.post("/import-record/:id", async (req, res) => {
    try {
        ...
        const device = customerDeviceRepo.create({
            ...
            deviceType: record.recordKind === "dasan_onu" ? "onu" : parseOptionalString(req.body?.deviceType),
            ipAddress: record.ipAddress ?? undefined,
            macAddress: record.macAddress ?? undefined,
...
router.post("/import-lease", async (req, res) => {
    try {
        ...
        const device = customerDeviceRepo.create({
            ...
            hostname,
            deviceType: parseOptionalString(req.body?.deviceType),
...
//# sourceMappingURL=network_discovery.js.map