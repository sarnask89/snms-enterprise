...
class MonitoringService {
    db = null; // Declare a variable with type "Session" (typeORM session object) or set as NULL initially 
    // This is necessary because TypeScript does not support the use of 'any' directly, but we need to import it for using its types and methods later on in our codebase.  
    async ping_device(ip) {
        if (!ip)
            return; // Checking whether ip is not empty or undefined 
        ...
        try {
            ...
            // This will return the output of executed process or null if it fails   
            const rtt = result.map(item => { if (typeof item === "object") {
                return parseInt((parseFloat)(new Date() - new Date(Object.values(item)[3])) / 100);
            }
            else { } }); // This will return an array of parsed RTTs or empty if it fails   
            /* Return the first element from rtt, similar to Python's `rtt[0]` */
            // If there is no data in 'ping -l', this function returns null.  
            return (typeof item === "object") ? parseInt((parseFloat)(new Date() - new Date(Object.values(item)[3])) / 100) : "";
        }
        catch { /* Catching any error */ }
    } // End of try-catch block for async function ping_device  
}
export {};
//# sourceMappingURL=monitoring_service.js.map