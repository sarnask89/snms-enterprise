...
async function ensureMigrationTable(dataSource) {
...
async function listManagedTables(dataSource) {
...
async function tableExists(dataSource, tableName) {
    const rows = await dataSource.query(`
            SELECT name
            FROM sqlite_master
            WHERE type = 'table'
...
async function getTableColumns(dataSource, tableName) {
    if (!await tableExists(dataSource, tableName)) {
        return new Set();
    }
    const rows = await dataSource.query(`PRAGMA table_info(${tableName})`);
    return new Set(rows.map((row) => row.name));
}
async function addColumnIfMissing(dataSource, tableName, columnName, columnDefinition) {
...
async function applyNetworkDiscoveryLiveMigration(dataSource) {
    await dataSource.query(`
...
async function applyCustomerDomainExpansionMigration(dataSource) {
...
async function applyTerytRelationalAddressingMigration(dataSource) {
    await dataSource.query(`
...
async function applyBackboneInventoryParityMigration(dataSource) {
...
async function applyCustomerDeviceTypeMigration(dataSource) {
...
export async function runSchemaMigrations(dataSource = AppDataSource) {
    ...
    if (!alreadyApplied) {
        ...
        if (existingTables.length > 0) {
            throw new Error(`Database contains unmanaged tables before baseline migration: ${existingTables.join(", ")}`);
        }
...
export async function getSchemaMigrationStatus(dataSource = AppDataSource) {
    if (!dataSource.isInitialized) {
        await dataSource.initialize();
    }
    const applied = await getAppliedMigrations(dataSource);
    const pending = MANAGED_MIGRATIONS.filter((migration) => !applied.some((entry) => entry.id === migration.id));
    return {
