var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Entity, PrimaryGeneratedColumn, Column } from "typeorm";
let PortalUser = class PortalUser {
    id;
    username;
    role;
    active;
}; // Defining the structure of PortalUser model.  
__decorate([
    PrimaryGeneratedColumn('uuid'),
    __metadata("design:type", String)
], PortalUser.prototype, "id", void 0);
__decorate([
    Column({ length: 50 }),
    __metadata("design:type", String)
], PortalUser.prototype, "username", void 0);
__decorate([
    Column("int"),
    __metadata("design:type", Number)
], PortalUser.prototype, "role", void 0);
__decorate([
    Column("boolean", { default: true }),
    __metadata("design:type", Boolean)
], PortalUser.prototype, "active", void 0);
PortalUser = __decorate([
    Entity() // This decorator is used for TypeORM to recognize this class as an entity in the database. 
], PortalUser);
export default PortalUser;
let PortalUserGroup = class PortalUserGroup {
    id;
    name;
};
__decorate([
    PrimaryGeneratedColumn('uuid'),
    __metadata("design:type", String)
], PortalUserGroup.prototype, "id", void 0);
__decorate([
    Column({ length: 50 }),
    __metadata("design:type", String)
], PortalUserGroup.prototype, "name", void 0);
PortalUserGroup = __decorate([
    Entity()
], PortalUserGroup);
export { PortalUserGroup };
//# sourceMappingURL=admin_views.js.map