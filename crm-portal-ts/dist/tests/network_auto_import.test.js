...
test("network discovery session auto-import creates parsed and fallback customers with tariff/subscription mapping", async (t) => {
    ...
    const autoImportPayload = await autoImportResponse.json();
    ...
    const autoCustomers = await autoCustomersResponse.json();
    ...
    const normalCustomers = await normalCustomersResponse.json();
    ...
    const secondAutoImportPayload = await secondAutoImportResponse.json();
...
//# sourceMappingURL=network_auto_import.test.js.map