...
test("TERYT XML import builds commune relations, default area and syncs customer/device address text", async (t) => {
    ...
    const defaultArea = await defaultAreaResponse.json();
    ...
    const cityRows = await citySuggest.json();
    ...
    const streetRows = await streetSuggest.json();
    ...
    const devicePayload = await createdDevice.json();
...
//# sourceMappingURL=teryt_import_relations.test.js.map