...
export async function batchResolveTerytAddresses(inputs) {
    ...
    const streetIds = Array.from(new Set(inputs.map((i) => i.streetId).filter((id) => typeof id === "number")));
    const cityIds = Array.from(new Set(inputs.map((i) => i.cityId).filter((id) => typeof id === "number")));
    const communeIds = Array.from(new Set(inputs.map((i) => i.communeId).filter((id) => typeof id === "number")));
    const districtIds = Array.from(new Set(inputs.map((i) => i.districtId).filter((id) => typeof id === "number")));
    const stateIds = Array.from(new Set(inputs.map((i) => i.stateId).filter((id) => typeof id === "number")));
