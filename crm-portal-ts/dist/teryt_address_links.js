...
export async function batchResolveTerytAddresses(inputs) {
    ...
    const streetIds = [...new Set(inputs
    ...
    const cityIdsNeeded = [...new Set(inputs
            .map((input) => input.cityId)
            .filter((id) => typeof id === "number" && id > 0 && !citiesMap.has(id)))];
    ...
    const communeIdsNeeded = [...new Set(inputs
            .map((input) => input.communeId)
            .filter((id) => typeof id === "number" && id > 0 && !communesMap.has(id)))];
    ...
    const districtIdsNeeded = [...new Set(inputs
            .map((input) => input.districtId)
            .filter((id) => typeof id === "number" && id > 0 && !districtsMap.has(id)))];
    ...
    const stateIdsNeeded = [...new Set(inputs
            .map((input) => input.stateId)
            .filter((id) => typeof id === "number" && id > 0 && !statesMap.has(id)))];
