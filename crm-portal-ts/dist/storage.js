...
export async function saveDocumentBase64(contentBase64, originalFilename, mimeType) {
...
export async function readStoredDocument(relativePath) {
    const absolutePath = resolveStoredDocumentPath(relativePath);
    const content = await readFile(absolutePath);
    return { absolutePath, content };
}
export function resolveStoredDocumentPath(relativePath) {
...
export async function deleteStoredDocument(relativePath) {
    if (!relativePath) {
        return;
    }
    try {
