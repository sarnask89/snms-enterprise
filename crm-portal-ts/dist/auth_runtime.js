...
export function establishPortalSession(response, userId) {
    const expiresAt = Date.now() + SESSION_TTL_MS;
    const token = signToken(userId, expiresAt);
    const secureFlag = isProductionRuntime() ? "; Secure" : "";
    response.setHeader("Set-Cookie", `${COOKIE_NAME}=${token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${Math.floor(SESSION_TTL_MS / 1000)}${secureFlag}`);
}
export function clearPortalSession(response) {
    const secureFlag = isProductionRuntime() ? "; Secure" : "";
    response.setHeader("Set-Cookie", `${COOKIE_NAME}=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0${secureFlag}`);
}
...
//# sourceMappingURL=auth_runtime.js.map