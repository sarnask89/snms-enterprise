  ...
  <div class="p-8 max-w-7xl mx-auto">
    <div class="flex items-center justify-between mb-8">
      ...
      <div class="flex gap-3">
        <UButton to="/finances" color="neutral" variant="soft" icon="i-lucide-banknote" label="Finanse" />
...
const saveSubscription = async () => {
  ...
  try {
    ...
    if (form.id) {
      ...
      toast.add({ title: 'Subskrypcja zaktualizowana', color: 'success' })
    } else {
      await $fetch('/api/v1/subscriptions', { method: 'POST', body: payload })
      toast.add({ title: 'Subskrypcja utworzona', color: 'success' })
    }
    ...
    toast.add({ title: 'Błąd zapisu', description: err.message, color: 'error' })
  } finally {
...
const toggleSubscription = async (row) => {
  try {
    ...
    toast.add({ title: 'Status zmieniony', color: 'success' })
    await refreshSubscriptions()
  } catch (err) {
    toast.add({ title: 'Błąd zmiany statusu', description: err.message, color: 'error' })
  }
}
...
const removeSubscription = async (row) => {
  ...
  try {
    ...
    toast.add({ title: 'Błąd usuwania', description: err.message, color: 'error' })
  }
}
</script>
