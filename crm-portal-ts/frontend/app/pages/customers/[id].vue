    ...
    <section class="space-y-4">
      <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-2">
          <UBadge :color="form.customerType === 'company' ? 'primary' : 'neutral'" variant="soft">
            {{ form.customerType === 'company' ? 'firma' : 'indywidualny' }}
          </UBadge>
          <UBadge :color="statusColor(form.status)" variant="soft">
            {{ statusLabel(form.status) }}
          </UBadge>
          <UBadge :color="form.isAutoGenerated ? 'warning' : 'success'" variant="soft">
            {{ form.isAutoGenerated ? 'auto-import' : 'manualny' }}
          </UBadge>
          <UButton icon="i-lucide-arrow-left" color="neutral" variant="ghost" to="/customers" label="Wróć do listy" aria-label="Wróć do listy abonentów" />
        </div>
      </div>
    ...
    <form v-if="customer" class="grid gap-6 xl:grid-cols-[minmax(0,1fr)_20rem]" @submit.prevent="saveCustomer">
      <section class="space-y-4">
        ...
        <UCard :ui="{ base: 'rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-950', header: { padding: 'px-5 py-4' }, body: { padding: 'px-5 py-5' } }">
          <template #header>
            <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <UButton size="sm" color="neutral" variant="soft" label="Użyj domyślnego miasta" @click="applyDefaultAreaToCustomer" aria-label="Użyj domyślnego miasta jako adresu korespondencyjnego" />
            </div>
          </template>
        ...
        <UCard :ui="{ base: 'rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-950', header: { padding: 'px-5 py-4' }, body: { padding: 'px-5 py-5' } }">
          <template #header>
            <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <UButton v-if="customer.devices?.length" size="xs" color="neutral" variant="soft" label="Odśwież klienta" @click="refresh" aria-label="Odśwież profil klienta" />
            </div>
          </template>
            ...
            <div class="space-y-3">
              ...
              >
                <div class="flex items-start justify-between gap-3">
                  ...
                  <UButton size="xs" color="neutral" variant="soft" label="Edytuj adres" @click="openDeviceEditor(device)" :aria-label="`Edytuj adres instalacji dla urządzenia ${device.hostname}`" />
                </div>
              </div>
              </div>
            ...
            <div class="rounded-lg border border-gray-200 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-900/70">
              ...
              <form v-if="isDeviceEditorOpen" class="mt-4 space-y-4" @submit.prevent="saveDeviceAddress">
                ...
                <div class="flex items-center justify-between gap-3">
                  <UButton size="sm" color="neutral" variant="soft" label="Użyj domyślnego miasta" @click="applyDefaultAreaToDevice" aria-label="Użyj domyślnego miasta jako adresu instalacji urządzenia" />
                  <div class="flex gap-2">
                    <UButton color="neutral" variant="ghost" label="Anuluj" @click="isDeviceEditorOpen = false" />
                    <UButton type="submit" color="primary" :loading="isSavingDevice" label="Zapisz adres" aria-label="Zapisz adres instalacji urządzenia" />
                  </div>
                </div>
              </form>
        ...
        <div class="flex justify-end gap-3">
          <UButton color="neutral" variant="ghost" to="/customers" label="Anuluj" />
          <UButton type="submit" color="primary" :loading="isSaving" label="Zapisz zmiany" aria-label="Zapisz wszystkie zmiany profilu klienta" />
        </div>
      </section>
...
const saveDeviceAddress = async () => {
  ...
  try {
    ...
    toast.add({
      title: 'Adres urządzenia zapisany',
      description: 'Adres instalacyjny urządzenia został zaktualizowany.'
    })
...
const saveCustomer = async () => {
  ...
  try {
    ...
    toast.add({
      ...
      description: 'Zmiany w profilu klienta zostały pomyślnie zaktualizowane.'
    })
    await refresh()
  } catch (error) {
