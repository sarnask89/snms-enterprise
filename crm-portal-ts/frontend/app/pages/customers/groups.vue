...
const memberPreview = (row) => {
  ...
  const shown = row.customers
    .slice(0, 3)
    .map((customer) => `${customer.firstName} ${customer.lastName}`)
    .join(', ')

  if (row.customers.length > 3) {
    return `${shown} i ${row.customers.length - 3} więcej`
  }
...
const saveGroup = async () => {
  ...
  try {
    ...
    toast.add({
      title: form.id ? 'Grupa zaktualizowana' : 'Grupa dodana',
      description: `Grupa ${form.name} została pomyślnie zapisana.`
    })
    ...
    toast.add({
      title: 'Błąd zapisu',
      description: error.data?.message || 'Wystąpił nieoczekiwany błąd.',
      color: 'error'
    })
  } finally {
