<template>
  <div class="p-8 max-w-7xl mx-auto space-y-8">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      ...
      <UButton color="neutral" variant="ghost" icon="i-lucide-refresh-cw" label="Odśwież" aria-label="Odśwież monitoring" @click="refreshAll" />
    </div>
    ...
    <div class="grid xl:grid-cols-2 gap-6">
      <UCard>
        ...
        <div class="space-y-4">
          ...
          />
          <UButton color="primary" :loading="isLoadingDeviceStats" label="Pobierz statystyki urządzenia" aria-label="Pobierz statystyki urządzenia" @click="loadDeviceStats" />
      ...
      <UCard>
        ...
        <div class="space-y-4">
          <UInput v-model="customerDeviceId" type="number" placeholder="ID urządzenia klienta" aria-label="ID urządzenia klienta" />
          <UButton color="primary" variant="soft" :loading="isLoadingCustomerStats" label="Pobierz statystyki klienta" aria-label="Pobierz statystyki klienta" @click="loadCustomerStats" />
...
const loadDeviceStats = async () => {
  ...
  try {
    ...
    toast.add({
      ...
      description: 'Dane urządzenia sieciowego zostały zaktualizowane.'
    })
  } catch (error) {
    toast.add({
      title: 'Błąd pobierania',
      description: error.data?.message || 'Nie udało się pobrać statystyk urządzenia.',
      color: 'error'
    })
  } finally {
...
const loadCustomerStats = async () => {
  ...
  try {
    ...
    toast.add({
      title: 'Statystyki pobrane',
      description: 'Dane urządzenia klienta zostały zaktualizowane.'
    })
  } catch (error) {
    toast.add({
      title: 'Błąd pobierania',
      description: error.data?.message || 'Nie udało się pobrać statystyk klienta.',
      color: 'error'
    })
  } finally {
