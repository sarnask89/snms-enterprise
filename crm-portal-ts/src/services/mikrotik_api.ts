...
export type MikrotikAccessProfile = {
    host: string;
    port: number;
    username: string;
    password: string;
...
export type MikrotikNetworkRow = {
    address: string | null;
    gateway: string | null;
    comment: string | null;
};

export type MikrotikBridgeHostRow = {
    macAddress: string | null;
...
function createFixtureResponse() {
    return {
        ...
        ] satisfies MikrotikBridgeHostRow[],
    };
}
...
class RouterOsClient {
    ...
    private pendingResolvers: Array<(sentence: string[]) => void> = [];
    private pendingRejectors: Array<(error: Error) => void> = [];

    constructor(private readonly profile: MikrotikAccessProfile) {}
    ...
    async writeSentence(words: string[]) {
        if (!this.socket) {
...
export async function runMikrotikDiscovery(profile: MikrotikAccessProfile) {
