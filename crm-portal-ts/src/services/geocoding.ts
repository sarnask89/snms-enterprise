...
class GeocodingService{ /* Service for address geocoding using Nominatim (OpenStreetMap). */ 
     ...
     static USER_AGENT = 'SNMS-Enterprise-CRM/1.0'; 
        ...
        async geocodeAddress(city: string, street:string , number :number) { // Method to Geocodes a Polish address  
            if (!all([ city,street, number])){ return null;}   
             ...
             try{        let resp = await axios.get(this.BASE_URL, {params: params , headers :headers});    
               if (resp && resposnseData .length > 0){       return  {"lat": parseFloat((responseData[0]["lat"])), "lon":parseFloat (( responseData [0 ] [" lon"]))}; }    // Assuming you are using SQLAlchemy with TypeORM and have set up the lat, long parameters accordingly in your codebase  
            catch (e){       logger.error(`Geocoding failed for ${params['q']}:  $ { e} `);  return null;}}     ;    // Assuming you are using SQLAlchemy with TypeORM and have set up the error handling parameter accordingly in your codebase  
            } catch (e)      logger.error(`Geocoding failed for ${params['q']}:  $ { e} `);  return null;}}     ;    // Assuming you are using SQLAlchemy with TypeORM and have set up the error handling parameter accordingly in your codebase  
