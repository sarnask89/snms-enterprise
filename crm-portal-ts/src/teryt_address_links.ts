...
export async function batchResolveTerytAddresses(inputs: TerytIdInput[]): Promise<ResolvedTerytAddress[]> {
    ...
    const recordRelations = (
        ...
        if (street && !streetsMap.has(street.id)) streetsMap.set(street.id, street);
        if (city && !citiesMap.has(city.id)) citiesMap.set(city.id, city);
        if (commune && !communesMap.has(commune.id)) communesMap.set(commune.id, commune);
        if (district && !districtsMap.has(district.id)) districtsMap.set(district.id, district);
        if (state && !statesMap.has(state.id)) statesMap.set(state.id, state);
    };
    ...
    if (streetIds.size > 0) {
        ...
        for (const st of streets) {
            recordRelations(
                st,
                st.city,
                st.commune ?? st.city?.commune,
                st.commune?.district ?? st.city?.district,
                st.commune?.district?.state ?? st.city?.district?.state,
            );
        }
    }
    ...
    if (missingCityIds.length > 0) {
        ...
        for (const c of cities) {
            recordRelations(
                ...
                c,
                c.commune,
                c.district ?? c.commune?.district,
                c.district?.state ?? c.commune?.district?.state,
            );
        }
    }
