...
function parseInvoiceStatus(value: unknown, fallback = InvoiceStatus.draft) {
    const candidate = String(value ?? "").trim() as InvoiceStatus;
    return Object.values(InvoiceStatus).includes(candidate) ? candidate : fallback;
}

function parseInvoiceKind(value: unknown, fallback = InvoiceDocumentKind.invoice) {
    const candidate = String(value ?? "").trim() as InvoiceDocumentKind;
    return Object.values(InvoiceDocumentKind).includes(candidate) ? candidate : fallback;
}

function parseLedgerKind(value: unknown, fallback = LedgerEntryKind.debit) {
    const candidate = String(value ?? "").trim() as LedgerEntryKind;
    return Object.values(LedgerEntryKind).includes(candidate) ? candidate : fallback;
}

function parseDateString(value: unknown, fallback?: string) {
...
function serializeTariff(tariff: Tariff) {
    ...
    const subscriptionCount = (tariff as { subscriptionCount?: number }).subscriptionCount ?? subscriptions.length;
...
function serializeRecurringPayment(payment: RecurringPayment) {
    return {
        id: payment.id,
...
function serializeCashReceipt(receipt: CashReceipt) {
    return {
        id: receipt.id,
