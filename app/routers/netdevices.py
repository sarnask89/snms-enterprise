...
def netdevice_pon_port_view(dev_id: int, port_id: str, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        for line in macs_out.splitlines():
            # Use pre-compiled RE_ONU_MAC_LINE for performance optimization
            match = RE_ONU_MAC_LINE.search(line)
            if match:
                o_id = match.group(1)
                mac = match.group(2)
                vid = match.group(3)
        ...
        rx_map = {}
        # Pre-compile the dynamic pattern exactly once outside the processing loop
