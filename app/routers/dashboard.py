...
def dashboard_home(request: Request, db: Session = Depends(get_db)):
    try:
        # Optimized: Batching multiple count queries into a single database round-trip using scalar subqueries.
        ...
        counts_row = db.execute(select(
            q_customers, q_invoices, q_tariffs, q_tickets, q_documents, q_nodes, q_subs
        )).one()

        n_customers, n_invoices, n_tariffs, n_tickets_open, n_documents, n_nodes, n_subs = counts_row
