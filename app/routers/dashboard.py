...
def dashboard_home(request: Request, db: Session = Depends(get_db)):
    try:
        # Optimization: Batch multiple count queries into one to reduce database round-trips
        stats_stmt = select(
            ...
            ).scalar_subquery().label("subscriptions_active"),
        )
        stats = db.execute(stats_stmt).mappings().one()

        n_customers = stats["customers"]
        n_invoices = stats["invoices"]
        n_tariffs = stats["tariffs"]
        n_tickets_open = stats["tickets_open"]
        n_documents = stats["documents"]
        n_nodes = stats["nodes"]
        n_subs = stats["subscriptions_active"]
        # Fetch active alarms
