...
# Bolt optimization: Pre-compile regex patterns at module level to avoid re-compilation per search request
RE_MAC_PREFIX = re.compile(r'^[0-9a-f]{2}[:.-]')
RE_MAC_HEX = re.compile(r'^[0-9a-f]{4,12}$')
RE_IP_PREFIX = re.compile(r'^\d{1,3}\.')
...
def global_search(request: Request, q: str = Query(""), db: Session = Depends(get_db)):
    ...
    if q:
        ...
        if RE_MAC_PREFIX.search(clean_q) or RE_MAC_HEX.search(clean_q) or ":" in clean_q:
            search_type = "mac"
        # Look for IP-like: digits followed by dots
        elif RE_IP_PREFIX.search(clean_q) or (RE_DIGIT.search(clean_q) and "." in clean_q):
            search_type = "ip"
