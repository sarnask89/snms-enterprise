...
# Pre-compiled regular expressions for search type detection performance
RE_MAC_SEPARATED = re.compile(r'^[0-9a-f]{2}[:.-]')
RE_MAC_PLAIN = re.compile(r'^[0-9a-f]{4,12}$')
RE_IP_START = re.compile(r'^\d{1,3}\.')
...
def global_search(request: Request, q: str = Query(""), db: Session = Depends(get_db)):
    ...
    if q:
        ...
        # Look for MAC-like: hex pairs with separators or long hex strings using precompiled regex
        if RE_MAC_SEPARATED.search(clean_q) or RE_MAC_PLAIN.search(clean_q) or ":" in clean_q:
            search_type = "mac"
        # Look for IP-like: digits followed by dots using precompiled regex
        elif RE_IP_START.search(clean_q) or (RE_ANY_DIGIT.search(clean_q) and "." in clean_q):
            search_type = "ip"
