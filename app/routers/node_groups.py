...
router = APIRouter(prefix="/node-groups", dependencies=[Depends(verify_session)])


@router.get("", response_class=HTMLResponse)
def node_groups_list(request: Request, db: Session = Depends(get_db)):
    rows = list(db.scalars(select(models.NodeGroup).order_by(models.NodeGroup.name)).all())
    return render(
        request,
        "node_groups/list.html",
...
def node_group_add_alias():
    """Alias `/node-groups/add` — ten sam formularz co /node-groups/new."""
    return RedirectResponse("/node-groups/new", status_code=303)


@router.get("/new", response_class=HTMLResponse)
def node_group_new_form(request: Request, db: Session = Depends(get_db)):
    nodes = list(db.scalars(select(models.Node).order_by(models.Node.hostname)).all())
    return render(
        request,
        "node_groups/form.html",
...
async def node_group_new_submit(request: Request, db: Session = Depends(get_db)):
    ...
    g = models.NodeGroup(name=name[:128], description=desc)
    db.add(g)
    db.flush()
    _sync_group_nodes(db, g, form.getlist("node_id"))
    db.commit()
    return RedirectResponse("/node-groups", status_code=303)
...
def node_group_edit_form(group_id: int, request: Request, db: Session = Depends(get_db)):
    g = db.get(models.NodeGroup, group_id)
    if not g:
        return RedirectResponse("/node-groups", status_code=302)
    nodes = list(db.scalars(select(models.Node).order_by(models.Node.hostname)).all())
    selected = {n.id for n in g.nodes}
    return render(
        request,
        "node_groups/form.html",
        {
...
def _sync_group_nodes(db: Session, group: models.NodeGroup, raw_ids: list) -> None:
    ids: set[int] = set()
    for x in raw_ids:
        try:
            ids.add(int(x))
            ...
            continue
    chosen = [db.get(models.Node, i) for i in ids]
    group.nodes = [n for n in chosen if n is not None]
