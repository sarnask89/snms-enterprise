...
def subscription_list(request: Request, db: Session = Depends(get_db)):
    # Optimized subscription query using joinedload to fetch associated Customer, Tariff
    # (including nested VatRate for gross price calculation), and CustomerDevice (device)
    # in a single query pass. This eliminates redundant full-table dictionary fetches
    # and reduces the N+1 query problem significantly.
    stmt = (
...
def subscription_new_form(
    ...
    initial_nodes = []
    if customer_id:
        initial_nodes = list(db.scalars(select(models.CustomerDevice).where(models.CustomerDevice.customer_id == customer_id)).all())
...
def get_customer_nodes(customer_id: int, db: Session = Depends(get_db)):
    nodes = list(db.scalars(select(models.CustomerDevice).where(models.CustomerDevice.customer_id == customer_id)).all())
