...
def subscription_list(request: Request, db: Session = Depends(get_db)):
    # Optimize by using joinedload to eager load customer and tariff records in a single query
    # rather than loading all customer and tariff tables in separate massive lookups.
    stmt = (
        select(models.Subscription)
        .options(joinedload(models.Subscription.customer), joinedload(models.Subscription.tariff))
