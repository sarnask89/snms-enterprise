...
def _render_node_form(
    ...
    db: Session,
    *,
    device: models.CustomerDevice | None = None,
    ...
    if net_id is None and device and device.ip_network_id:
        net_id = device.ip_network_id
    exclude = device.id if device else None
    ip_suggestions: list[str] = []
    if net_id:
        ip_suggestions = free_ipv4_suggestions(db, net_id, exclude_node_id=exclude)
    title = (
        f"Edycja komputera: {device.hostname}"
        if device
...
def node_ip_suggestions_partial(
    ...
    ex = _opt_int(device_id)
    if not nid:
        return HTMLResponse('<datalist id="device-ip-suggestions"></datalist>')
    ips = free_ipv4_suggestions(db, nid, exclude_node_id=ex)
    opts = "".join(f'<option value="{html.escape(ip)}">' for ip in ips)
    return HTMLResponse(f'<datalist id="device-ip-suggestions">{opts}</datalist>')
...
def node_add_alias():
    """Alias `/customer_devices/add` — ten sam formularz co /customer-devices/new."""
...
def node_reports(request: Request, db: Session = Depends(get_db)):
    ...
    stmt = select(models.CustomerDevice).options(joinedload(models.CustomerDevice.customer)).order_by(models.CustomerDevice.hostname)
...
def node_print_alias():
    """Alias `/customer_devices/print` — przekierowanie do raportu (druk / lista)."""
...
def node_list(
    ...
    stmt = select(models.CustomerDevice).options(joinedload(models.CustomerDevice.customer)).order_by(models.CustomerDevice.id)
    if q and q.strip():
        term = f"%{q.strip()}%"
        cust_ids = [
            r[0]
        ...
        parts = [
            models.CustomerDevice.hostname.ilike(term),
            models.CustomerDevice.ip_address.ilike(term),
            models.CustomerDevice.mac_address.ilike(term),
        ]
        if cust_ids:
...
def node_session_delete(session_id: int, db: Session = Depends(get_db)):
    s = db.get(models.CustomerDeviceSession, session_id)
    if s:
        db.delete(s)
        db.commit()
    return RedirectResponse("/customer-devices/sessions", status_code=303)
...
def node_notice_delete(notice_id: int, db: Session = Depends(get_db)):
    n = db.get(models.CustomerDeviceNotice, notice_id)
    if n:
        db.delete(n)
        db.commit()
    return RedirectResponse("/customer-devices/notices", status_code=303)
...
def node_new_form(
    ...
    if customer_id or mac_address or ip_address or net_device_id:
        sticky = {
            ...
            "name": f"device-{mac_address.replace(':', '')[-4:]}" if mac_address else ""
        }
    return _render_node_form(request, db, device=None, sticky=sticky)
...
def node_new_submit(
    ...
    if not ok:
        ...
        return _render_node_form(
            request, db, device=None, sticky=sticky, ip_error=err, status_code=400
        )
    n = models.CustomerDevice(
        customer_id=customer_id,
        name=name.strip(),
        hostname=hostname,
...
def node_edit_form(device_id: int, request: Request, db: Session = Depends(get_db)):
    n = db.get(models.CustomerDevice, device_id)
    if not n:
        return RedirectResponse("/customer-devices", status_code=302)
    return _render_node_form(request, db, device=n)


@router.post("/{device_id}/edit", dependencies=[Depends(require_business_write)])
def node_edit_submit(
    ...
    ok, err = validate_node_ip_assignment(
        db, net_id, ip_address, exclude_node_id=device_id
    )
    if not ok:
        sticky = {
            "customer_id": customer_id,
        ...
        return _render_node_form(
            request, db, device=n, sticky=sticky, ip_error=err, status_code=400
        )
    ...
    n.status = models.CustomerDeviceStatus(status)
...
@router.post("/{device_id}/delete", dependencies=[Depends(require_business_write)])
def node_delete(device_id: int, db: Session = Depends(get_db)):
    n = db.get(models.CustomerDevice, device_id)
    if n:
        db.delete(n)
        db.commit()
