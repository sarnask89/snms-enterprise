...
# Precompiled regex patterns for parsing CLI output at module level to avoid recompilation overhead
ONU_ACTIVE_LINE_RE = re.compile(r"^\s*(\d+)\s*\|\s*(\d+)\s*\|")
OLT_MAC_LINE_RE = re.compile(r"^\s*\d+\s*\|\s*(\d+)\s*\|\s*(\d+)\s*\|\s*([0-9a-fA-F:]{17})\s*\|\s*\d+\s*\|\s*(\d+)")
ONU_STATUS_RE = re.compile(r"\|\s*(\d+)\s*\|\s*(\w+)\s*\|")
BRIDGE_MAC_LINE_RE = re.compile(r"^\s*(\d+)\s+([a-zA-Z0-9/]+)\s+([0-9a-fA-F:]+)")
PARSE_ONU_LINE_RE = re.compile(r"^\s*(\d+)\s*\|\s*(\d+)\s*\|\s*(\w+)\s*\|\s*\w+\s*\|\s*(\w+)")
PARSE_MACS_ETH_RE = re.compile(r"^\s*(\d+)\s+(eth\d+)\s+([0-9a-fA-F:]{17})")
PARSE_MACS_OLT_RE = re.compile(r"^\s*\d+\s*\|\s*\d+\s*\|\s*\d+\s*\|\s*([0-9a-fA-F:]{17})\s*\|\s*(\d+)\s*\|\s*(\d+)\s*\|\s*(\w+)")
ONU_DETAILS_RE = re.compile(r"\|\s*\d+\s*\|\s*(\w+)\s*\|\s*\w+\s*\|\s*(\w+)\s*\|[^\|]+\|\s*([0-9:]+)")
DBM_SIGNAL_RE = re.compile(r"(-?\d+\.\d+\s*dBm)")
...
class DasanService:
    ...
    def get_onu_path(self, mac_address: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for line in onu_active_raw.splitlines():
                ...
                match = ONU_ACTIVE_LINE_RE.match(line)
                if match:
            ...
            for olt_port in active_olt_ports:
                ...
                for line in output_olt.splitlines():
                    if mac_formatted in line.lower():
                        ...
                        match = OLT_MAC_LINE_RE.match(line)
                        if match:
            ...
            for line in lines:
                if mac_formatted in line.lower():
                    ...
                    match_data = BRIDGE_MAC_LINE_RE.match(line)
                    if match_data:
                        return {
    ...
    def _parse_onu(self, raw: str) -> List[Dict[str, str]]:
        ...
        for line in lines:
            match = PARSE_ONU_LINE_RE.match(line)
            if match:
                onus.append({
                    "olt": match.group(1),
                    "onu_id": match.group(2),
    ...
    def _parse_macs(self, raw: str) -> List[Dict[str, str]]:
        ...
        for line in lines:
            ...
            match1 = PARSE_MACS_ETH_RE.match(line)
            if match1:
                macs.append({
                    "vid": match1.group(1),
                    "port": match1.group(2),
            ...
            match2 = PARSE_MACS_OLT_RE.match(line)
            if match2:
                macs.append({
                    "mac": match2.group(1),
    ...
    def get_onu_details(self, olt_port: str, onu_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for line in out_active.splitlines():
                if f"| {onu_id} |" in line:
                    match = ONU_DETAILS_RE.search(line)
                    if match:
            ...
            for line in out_power.splitlines():
                ...
                if f"{olt_port}/{onu_id}" in line:
                    p_match = DBM_SIGNAL_RE.search(line)
                    if p_match:
