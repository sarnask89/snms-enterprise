...
def test_get_customer_nodes_htmx(admin_client, session):
    ...
    n1 = models.CustomerDevice(customer_id=c.id, hostname="node-1", status=models.CustomerDeviceStatus.active)
    n2 = models.CustomerDevice(customer_id=c.id, hostname="node-2", status=models.CustomerDeviceStatus.active)
...
def test_subscription_links_to_node(admin_client, session):
    ...
    n = models.CustomerDevice(customer_id=c.id, hostname="specific-node", status=models.CustomerDeviceStatus.active)
    session.add(n)
    session.flush()
